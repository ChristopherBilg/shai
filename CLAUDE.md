...
`gh api repos/<repo>/issues/<number>/dependencies/blocked_by` — if any blocking issue has
`state != "closed"`, the issue is deferred (label stays, next tick re-checks); if the API call
fails, the issue is also deferred (fail-safe). Unblocked issues proceed: checks the
...
(next tick re-checks); dependency API failure → defer with warning (next tick retries); label
