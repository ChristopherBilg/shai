...
`gh_pr_comments`, `jira_issue_view`, `list_directory`, `print_file` (read-only),
`write_file`, `patch_file`, `delete_file`, `gh_pr_review_create`, `gh_pr_review_submit`,
`gh_repo_clone`, `git_checkout_pr` (write). `shai-tools`
