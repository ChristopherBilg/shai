...
assert_eq "$(find "$PHOME/sessions/" -mindepth 1 -maxdepth 1 | wc -l | tr -d ' ')" "2" \
...
assert_eq "$(find "$PHOME/sessions/" -mindepth 1 -maxdepth 1 2>/dev/null | wc -l | tr -d ' ')" "1" \
  "prune: --before removes only old session"
assert_eq "$(ls "$PHOME/sessions/")" "sess_b.jsonl" \
  "prune: --before keeps recent session"
