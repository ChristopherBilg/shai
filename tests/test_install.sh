...
assert_contains "$OUT" "Add to .zshrc: fpath=" \
  "install: a newly created zsh completion dir prints the setup instruction"
assert_contains "$OUT" "Installed bash completions to" \
  "install: the bash completion install prints its success line"
assert_contains "$OUT" "Add to .bashrc:" \
  "install: a newly created bash completion dir prints the setup instruction"
