...
# wfro: names a read-only tool — it needs an explicit grant too, because shai-dispatch's
# per-tool read-only fallback is not guaranteed (a base default/rule can pre-empt it).
echo '{"rules":[{"tool":"gamma","action":"allow"}]}' >"$WF/workflows/wfro/policy.json"
...
assert_contains "$OUT" "prompts/wfro.txt names only tools" "explicitly granted read-only tool → per-prompt ok line"
...
# --- a read-only tool with no explicit grant fails — no fallback to hide behind ---
echo '{"rules":[]}' >"$WF/workflows/wfro/policy.json"
OUT="$(bash "$DIR/tests/tools-sync.sh" "$WF" 2>&1)"
assert_eq "$?" "1" "read-only tool without an explicit grant → exit 1"
assert_contains "$OUT" "does not grant: gamma" "no fallback → read-only tool reported"
