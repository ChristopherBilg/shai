...
_CLEANUP_DIRS+=("$FIX")
mkdir -p "$FIX/tests"
cp "$DIR/tests/conventions.sh" "$FIX/conventions.sh"
...
git -C "$FIX" add conventions.sh shai-clean tests/lint.sh tests/run.sh tests/check-untracked.sh
...
OUT="$(bash "$FIX/conventions.sh" 2>&1)"
