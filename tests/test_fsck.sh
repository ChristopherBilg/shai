...
#         protection, scoping, the dry-run/fix plan identity (including the dry-run
#         refusal of a rebuild the real --fix cannot complete), exit codes 0/1/3), usage
...
DRY_ERR=$("$FSCK" --fix --dry-run 2>&1 >/dev/null)
assert_eq "$?" "1" "--fix --dry-run on the blocked rebuild does not exit 0"
assert_not_contains "$DRY_ERR" "repair: rebuild" \
  "--fix --dry-run prints no rebuild plan the real --fix would refuse"
