...
make_run_with_dumps() {
  ...
  printf '{"model":"test-model","messages":[]}' >"$SHAI_HOME/runs/$run_id/span_1-request.json"
  printf '{"message_id":"msg_1","model":"test-model","usage":{"prompt_tokens":10,"completion_tokens":5,"total_tokens":15},"finish_reason":"stop","latency_ms":100}' \
...
make_run_with_dumps "$RID" \
  ...
  "$(fixture_event "message" "assistant" '{"content":"hi there","finish_reason":"stop"}' \
    ...
    '{"message_id":"msg_1","model":"test-model","usage":{"prompt_tokens":10,"completion_tokens":5,"total_tokens":15},"latency_ms":100}')"
OUT=$("$TRACE" "$RID")
...
make_run_with_dumps "$RID" \
  ...
  "$(fixture_event "message" "assistant" '{"content":"cached reply","finish_reason":"stop"}' \
    "$RID" "sess_test" "span_1" \
    '{"message_id":"msg_1","model":"test-model","usage":{"prompt_tokens":100,"completion_tokens":20,"total_tokens":120,"prompt_cache_hit_tokens":80,"prompt_cache_miss_tokens":20},"latency_ms":50}')"
OUT=$("$TRACE" "$RID")
...
make_run_with_dumps "$RID" \
  ...
  "$(fixture_event "message" "assistant" '{"content":"no cache","finish_reason":"stop"}' \
    "$RID" "sess_test" "span_1" \
    '{"message_id":"msg_1","model":"test-model","usage":{"prompt_tokens":50,"completion_tokens":10,"total_tokens":60},"latency_ms":100}')"
OUT=$("$TRACE" "$RID")
CACHED_IN_OUTPUT=no
