#!/bin/bash
# test_heartbeat.sh — unit tests for workflows/heartbeat.sh
# Covers: workflows/heartbeat.sh — pipeline exercise, pass/fail reporting
set -uo pipefail
# shellcheck source=tests/lib.sh
source "$(dirname "${BASH_SOURCE[0]}")/lib.sh"
echo "workflows/heartbeat.sh"
...
OUT=$("$DIR/workflows/heartbeat.sh" 2>&1)
RC=$?
...
# workflows/heartbeat.sh sources lib/workflow.sh via "$(dirname "$0")/../lib/workflow.sh", and
...
mkdir -p "$PIPEDIR/lib" "$PIPEDIR/workflows" "$PIPEDIR/prompts"
cp "$DIR/lib/workflow.sh" "$PIPEDIR/lib/workflow.sh"
cp "$DIR/workflows/heartbeat.sh" "$PIPEDIR/workflows/heartbeat.sh"
...
chmod +x "$PIPEDIR/workflows/heartbeat.sh" "$PIPEDIR"/shai-*
# shai-loop is deliberately not copied here.

OUT=$("$PIPEDIR/workflows/heartbeat.sh" 2>&1)
RC=$?
