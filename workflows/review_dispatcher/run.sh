...
# Reads: gh auth from environment; searches open PRs labeled shai-review-dispatcher
...
SEARCH_JSON=$(gh search prs --label "$LABEL" --state open --limit 100 --json repository,number) ||
...
wf_output "dispatched=$DISPATCHED failed=$FAILED"
