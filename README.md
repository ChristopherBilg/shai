...
- `search_files` — search for a text pattern across files in a directory tree; returns `path:line_number: text` rows; supports `glob`, `ignore_case`, and `max_results`; skips binary files and `.git/` (read-only)
