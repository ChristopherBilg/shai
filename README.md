...
- `search_files` — search for a text pattern across files in a directory tree; returns `path:line_number: text` rows plus a `[truncated: showing first N matches]` marker when the cap is hit; supports `glob`, `ignore_case`, and `max_results`; skips binary files and default-excluded paths (`.git`, `.ssh`, `.env`, `node_modules`, …) (read-only)
