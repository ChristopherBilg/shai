...
Installs to `~/.local/share/shai/<version>/` with wrappers in `~/.local/bin/`. Requires `gh` CLI, authenticated. Upgrade by re-running. Rollback: `export SHAI_VERSION=v2026.08.09 && gh api ... | base64 -d | bash`.
