...
- `search_files` — search for a text pattern across files in a directory tree; returns `path:line_number: text` rows plus a `[truncated: showing first N matches]` marker when the cap is hit; supports `glob`, `ignore_case`, and `max_results`; skips binary files and default-excluded paths (`.git`, `.ssh`, `.env`, `node_modules`, …); a zero-match pattern containing regex metacharacters gets a `[note: 0 matches …]` line so an empty result is never silently misread (read-only)
